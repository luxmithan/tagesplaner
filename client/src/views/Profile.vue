  ...
  <v-col cols="12" sm="8" md="6">
    <v-card>
      <v-toolbar color="primary" dark flat>
        ...
        <v-row
          v-for="(value, propertyName, index) in myProfile"
          :key="index"
          no-gutters
        >
          <v-col cols="4">
        ...
        <v-form class="ma-4" ref="form">
          ...
          <v-alert v-if="success" dense outlined type="success">Ihr Passwort wurde erfolgreich verändert.</v-alert>
        </v-form>
...
export default {
  ...
  data: () => ({
    ...
    success: false,
    oldPasswordRules: [
      v => !!v || "Bitte altes Password eingeben",
    ],
    newPasswordRules: [
  ...
  methods: {
    ...
    async changePassword() {
      try {
        ...
        let data = await axios.put(`/api/user/changePassword`, passwordData)
    ...
    async deleteUser() {
      if (confirm('Sind Sie sicher, dass Sie ihr Konto inklusive alle dazugehörigen Ziele löschen wollen?')) {
